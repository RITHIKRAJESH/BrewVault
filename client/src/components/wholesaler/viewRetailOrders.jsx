...
export default function ViewRetailOrders() {
    ...
    const userId = localStorage.getItem('userId'); // Get the userId from localStorage (although it's no longer being used to filter)

    useEffect(() => {
        // Fetch orders without filtering by userId
        axios
            .get(`${import.meta.env.VITE_BASE_URL}/retailer/fetchOrder`)
            .then((res) => {
    ...
    const handleStatusChange = (orderId, newStatus) => {
        ...
        axios
            .put(`${import.meta.env.VITE_BASE_URL}/retailer/updateOrderStatus`, {
                orderId: orderId,
                status: newStatus
            })
            .then((response) => {
