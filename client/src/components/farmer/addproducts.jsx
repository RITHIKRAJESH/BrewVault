...
export default function AddProductDetails() {
  ...
  const addCategory = () => {
    const { quality, price } = product.productCategory[product.productCategory.length - 1];
    if (!quality || !price) {
      setMessage("Quality and Price must be filled before adding another category.");
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!product.productName || !product.productImage || !product.date || !product.quantity) {
      setMessage("All fields are required!");
      return;
    }
    
    if (!product.productCategory.every((cat) => cat.quality && cat.price)) {
      setMessage("All categories must have a valid quality and price.");
      return;
    }
  ...
  const handleUpdate = async () => {
    if (!selectedProduct.productCategory.every((cat) => cat.quality && cat.price)) {
      setMessage("All categories must have a valid quality and price.");
      return;
    }
    try {
